...
class IMFWBOutlookFetcher:
    ...
    def _persist(self, forecasts: list[ExtractedForecast]) -> None:
        ...
        ``ON CONFLICT (report_id)`` non è applicabile. Garantiamo l'upsert
        rimuovendo l'eventuale riga precedente con lo stesso ``report_id``.
        """
        for f in forecasts:
            try:
                ...
                log.warning("imf_wb.persist_failed",
