...
Initial Jobless Claims (ICSA) Ã¨ uno dei leading indicators piÃ¹ tempestivi:
  Â· Salgono mediamente 6-9 mesi PRIMA delle recessioni conclamate
  Â· La media mobile 4 settimane (4wk MA) smootha la volatilitÃ  settimanale
  Â· Variazione YoY > +15% segnala deterioramento significativo
...
_PEAK_MOM_THRESHOLD  = 0.05   # Claims in salita > 5% MoM â†’ peak
_CONTRACTION_YOY     = 0.15   # Claims > 15% YoY â†’ contraction confermata
_TROUGH_MA_DECLINING = -0.03  # 4wk MA scende > 3% â†’ trough/recovery
...
class ClaimsCycleDetector:
    ...
    def _compute_signal(self, df: pd.DataFrame) -> ClaimsCycleSignal:
        ...
        # 4-week moving average (kernel di convoluzione â€” vettorizzato)
    ...
    def _classify_regime(
        current_ma: float,
        ma_history: np.ndarray,  # type: ignore[type-arg]
        yoy_pct: float | None,
        mom_pct: float | None,
    ) -> ClaimsCycleRegime:
        ...
        if mom_pct is not None and mom_pct > _PEAK_MOM_THRESHOLD * 100:
            # Piccolo rialzo â†’ peak; rialzo forte â†’ contraction
            if mom_pct > 8.0:
                return "contraction"
            return "peak"
        ...
        if len(ma_history) >= 8:
            ...
            if len(valid) >= 8:
                # Se la MA attuale Ã¨ sotto la media delle ultime 8 settimane
